// fallphysio SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from fallphysio/index.html · 139377 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "fallphysio" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Fallphysio v1.0.0 · prime 743 · MIT
// Sovereign session management + UK law engine for HCPC-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='fallphysio';const VERSION='1.0.0';const PRIME=743;const STORE='fallphysio';const CONFIG_V='fallphysio@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','sessions','advice','corpus','weaves','audit','settings'];
...
const T0_RULES=[
 ...
 {q:'CFA vs DBA?',a:'CFA (Conditional Fee Agreement): "no win, no fee" with success fee uplift (max 100% of base costs, capped at 25% of damages excl. future care in PI). Governed by Courts & Physical therapy Services Act 1990 s.58, CFA Order 2013. DBA (Damages-Based Agreement): payment is % of damages recovered (50% civil, 35% employment, 25% PI). DBA Regs 2013. CFA success fee recoverable from client only since LASPO 2012. Hybrid CFA/DBA unenforceable: Zuberi v Lexlaw [2021] EWCA Civ 16.'},
 {q:'HCPC conduct rules?',a:'HCPC Standards & Regulations 2019. Seven Principles: uphold rule of law, public trust, independence, honesty, integrity, equality, best interests of client. Code of Conduct for Physiotherapists. Code for Firms. Accounts Rules (client money). Reg 13.5: 6-year file retention. COLP / COFA mandatory per firm. Annual practising certificate renewal. 16hrs CPD per year. Indemnity insurance £2m/£3m minimum per claim.'},
 ...
 {q:'Conveyancing protocol?',a:'CSP Conveyancing Protocol 2019 (residential). Steps: client onboarding + ID, contract pack, searches (local, water/drainage, environmental, chancel), enquiries, mortgage offer, exchange (10% deposit, contract binding), completion (balance + keys), SDLT return within 14 days, Land Registry application within priority period. CQS accreditation for lenders. HCPC Warning Notice on dubious investment schemes.'},
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order physical therapy representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W005',name:'The Conflict Wall',archetype:'Sister firm acted for opposing party — instruct fresh',agents:['GUILD','EQUITY'],move:'Information barrier built before instruction — Bolkiah test compliance.',authorities:['Prince Jefri Bolkiah v KPMG [1999] 2 AC 222','HCPC Code of Conduct Pt 6','HCPC Standards & Regs 2019'],opposition_move:'Former client seeks injunction to restrain.',counter:'Demonstrate effective information barrier: physical, electronic, signed undertakings, no client crossover, fee splits.',why_it_wins:'Bolkiah requires real and substantial risk of disclosure; effective Chinese wall rebuts it.',example_case:'Re Z [2009] EWHC 3621 (Ch) — barrier upheld.'},
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Physical therapy Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
 ...
 {id:'W030',name:'The Physiotherapists Act Assessment',archetype:'Client disputes physiotherapists bill',agents:['GUILD','EQUITY'],move:'Physiotherapists Act 1974 s.70 — client right to assessment within 1 month automatic; up to 12 months with cause; rare beyond.',authorities:['Physiotherapists Act 1974 ss.69-70','CPR 46.10','Tim Martin Interiors v Akin Gump [2011] EWCA Civ 1574'],opposition_move:'Bill compliant (s.69 signed, delivered) and time has run.',counter:'Check formal validity strictly — narrative sufficient? — and "special circumstances" for late assessment under s.70(3).',why_it_wins:'s.70 favours client procedurally; one-fifth reduction shifts costs to physiotherapist.',example_case:'Tim Martin v Akin Gump (bill validity strict).'}
];
const CORPUS=[
 {id:'sa1974',title:'Physiotherapists Act 1974',area:'wills-probate',summary:'Foundational statute regulating physiotherapists bills (ss.69-70), client account (now HCPC Rules), and disciplinary jurisdiction.',keyProvisions:['s.69 form of bill','s.70 client right to assessment','s.74 county court assessment']},
 {id:'lsa2007',title:'Physical therapy Services Act 2007',area:'other',summary:'Modernised physical therapy services regulation; created Physical therapy Services Board, LeO, ABS structures; defined reserved physical therapy activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to physical therapy advice (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',sessionTab:'overview',activeSessionId:null,
 firm:null,advisers:[],clients:[],sessions:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 if(meta){state.activeSessionId=meta.activeSessionId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.sessionTab=meta.sessionTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeSessionId:state.activeSessionId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,sessionTab:state.sessionTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const sessionId=(payload&&payload.sessionId)||state.activeSessionId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+sessionId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,sessionId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveSession(m,reason,noBc){m.updatedAt=Date.now();await idbPut('sessions',m);if(!state.sessions.find(x=>x.id===m.id))state.sessions.push(m);else state.sessions=state.sessions.map(x=>x.id===m.id?m:x);await audit('session.updated',reason||'Session updated',{id:m.id,sessionId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('session.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Physical therapy opinion signed and retained per HCPC 13.5',{id:a.id,sessionId:a.sessionId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,sessions:state.sessions});
 ...
 for(const mt of(p.sessions||[])){const local=state.sessions.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveSession(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='session.created'||m.type==='session.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.sessions.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveSession(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',sessionId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
// FACTORIES
// ════════════════════════════════════════════════════════════════
function newFirmRec(){return{id:'fm_'+uid(),createdAt:Date.now(),updatedAt:Date.now(),name:'',tradingName:'',sraNumber:'',companiesHouseNo:'',vatNumber:'',registeredAddress:{line1:'',line2:'',city:'',postcode:'',country:'GB'},piInsurer:'',piPolicyNo:'',piExpiresAt:null,professionalBody:'HCPC',brandColor:'#8b1a1a',brandLogoDataUri:'',setupCompletedAt:null,nextSessionRefSeq:1}}
function newAdviserRec(){return{id:'ad_'+uid(),firmId:state.firm&&state.firm.id,createdAt:Date.now(),updatedAt:Date.now(),archivedAt:null,name:'',email:'',phone:'',smcrRole:'physiotherapist',practicingCertNo:'',practicingCertExpiry:'',cpdHoursThisYear:0,cpdActivities:[],colp:false,cofa:false,status:'active',startedAt:Date.now(),leftAt:null}}
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForSession:''}},
...
function newSessionRec(clientId){
 const seq=(state.firm&&state.firm.nextSessionRefSeq)||(state.sessions.length+1);
 if(state.firm)state.firm.nextSessionRefSeq=seq+1;
 ...
 title:'New session',practiceArea:'civil-litigation',
 responsiblePhysiotherapistId:state.settings.currentAdviserId||'',supervisingPartnerId:'',
...
function activeSession(){return state.sessions.find(m=>m.id===state.activeSessionId)}
function sessionClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 const mt=newSessionRec(cl.id);
 mt.title='DEMO · Patel v Singh · RTA quantum · overwrite me';
 mt.practiceArea='personal-injury';mt.responsiblePhysiotherapistId=adviserId;mt.status='active';mt.feeArrangement='conditional';mt.cfaSuccessFeePct=25;mt.estimatedFees=12500;
 ...
 mt.demo=true;
 await saveSession(mt,'demo seed',true);
 state.activeSessionId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setSessionTab(id){state.sessionTab=id;saveMeta();render()}
function selectSession(id){state.activeSessionId=id;state.active='session';state.sessionTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activeSession();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no session selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const sessions=state.sessions.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(sessionClient(m)?clientLabel(sessionClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 if(f.area&&m.practiceArea!==f.area)return false;
 if(f.responsible&&m.responsiblePhysiotherapistId!==f.responsible)return false;
 ...
 <div class="session-list">${sessions.length?sessions.map(m=>{
 const cl=sessionClient(m);
 ...
 return `<div class="session-card ${state.activeSessionId===m.id?'active':''}" onclick="selectSession('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no sessions yet · click + new session</div>'}</div>`;
}
function render(){
 ...
 else if(a==='sessions')v.innerHTML=viewSessions();
 else if(a==='session')v.innerHTML=viewSession();
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallphysio</strong> is a tool for HCPC-regulated UK physiotherapists. It assists with session management, CDD, document generation, and HCPC Accounts Rules tracking. It is <strong>not regulatory submission or physical therapy opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Fallphysio</h1>
 <p class="lead">Sovereign session management + UK law research for 1-10 person HCPC-regulated firms. First, your firm details.</p>
 <div class="row"><div class="field"><label>Firm name *</label><input id="f_name" placeholder="Acme Physiotherapists LLP"></div><div class="field"><label>Trading name</label><input id="f_trading"></div></div>
 <div class="row"><div class="field"><label>HCPC number *</label><input id="f_sra" placeholder="123456"></div><div class="field"><label>Companies House</label><input id="f_ch"></div></div>
...
function onbStep2(){
 ...
 <h1>First adviser</h1>
 <p class="lead">Add yourself — HCPC roll number and which compliance roles you hold (COLP/COFA mandatory for the firm).</p>
 ...
 <div class="row"><div class="field"><label>HCPC roll number *</label><input id="a_sra"></div><div class="field"><label>Practising cert expiry</label><input type="date" id="a_exp"></div></div>
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>physiotherapist</option><option>partner</option><option>paraphysical therapy</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.sessions.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.sessions.filter(m=>m.status==='closed').length;
 const critical=state.sessions.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.sessions){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">HCPC ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.sessions.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no sessions</div>'}
 </div>
 ...
 <div class="kpi"><span class="l">Retention</span><span class="v brass">HCPC · 6 years</span></div>
 </div>
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectSession('${m.id}')">open →</button></div>`
...
function viewSessions(){
 const mm=state.sessions.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Sessions · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newSession()">+ new session</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=sessionClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectSession('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsiblePhysiotherapistId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no sessions · click + new session</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewSession(){
 const m=activeSession();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No session selected</h3><p>Choose one from the sidebar, or <a onclick="newSession()">create new</a>.</p></div>';
 const cl=sessionClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeSession('${m.id}')">${m.status==='closed'?'reopen':'close session'}</button>
 <button class="btn sm ox" onclick="deleteSession('${m.id}')">delete</button>
 </div>
 </div>
 <div class="session-tabs">${tabs.map(t=>`<button class="${state.sessionTab===t[0]?'active':''}" onclick="setSessionTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.sessionTab==='overview'?tabOverview(m,cl):''}
 ${state.sessionTab==='retainer'?tabRetainer(m):''}
 ${state.sessionTab==='conflict'?tabConflict(m,cl):''}
 ${state.sessionTab==='timeline'?tabTimeline(m):''}
 ${state.sessionTab==='advice'?tabAdvice(m):''}
 ${state.sessionTab==='documents'?tabDocs(m):''}
 ${state.sessionTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Session detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updSession('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updSession('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updSession('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updSession('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible physiotherapist</label><select onchange="updSession('responsiblePhysiotherapistId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsiblePhysiotherapistId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updSession('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updSession('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updSession('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updSession('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updSession('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updSession('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updSession('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','physical therapy-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updSession('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updSession('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updSession('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updSession('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updSession('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updSession('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updSession('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updSession('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local sessions + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (session)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForSession)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this session</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updSession('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updSession('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updSession('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.sessionId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue physical therapy opinion <span class="meta">HCPC 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>fallphysiopaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>fallphysiopractice</strong>. This view shows ledger ref IDs.</p>
...
function viewAdvisers(){
 ...
 <table class="tbl"><thead><tr><th>Name</th><th>Role</th><th>HCPC roll</th><th>PC expiry</th><th>COLP</th><th>COFA</th><th>CPD</th><th></th></tr></thead><tbody>
...
function viewFirm(){
 ...
 <div class="row"><div class="field"><label>HCPC number</label><input value="${esc(f.sraNumber)}" oninput="state.firm.sraNumber=this.value;saveFirm()"></div><div class="field"><label>Companies House</label><input value="${esc(f.companiesHouseNo)}" oninput="state.firm.companiesHouseNo=this.value;saveFirm()"></div></div>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic physical therapy patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeSessionId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeSessionId}','${w.id}')">apply to active session</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeSessionId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeSession().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Audit · P3</h2><div class="sub">${state.audit.length} entries · 6yr retention · HCPC 13.5</div><div class="actions"><button class="btn" onclick="exportAudit()">export JSON</button></div></div>
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / session.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Session</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.sessions.find(x=>x.id===a.sessionId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updSession(field,val){const m=activeSession();if(!m)return;m[field]=val;saveSession(m,'field '+field+' updated')}
async function newSession(){
 const m=newSessionRec();state.activeSessionId=m.id;
 await saveSession(m,'new session');await audit('session.created','New session created',{id:m.id,sessionId:m.id,ref:m.ref});broadcast('session.created',m);
 state.active='session';state.sessionTab='overview';saveMeta();render();
}
async function closeSession(id){const m=state.sessions.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveSession(m,'session '+m.status);if(m.status==='closed')broadcast('session.closed',m);else broadcast('session.reopened',m);render()}
async function deleteSession(id){if(!confirm('Delete this session? (audit log preserved)'))return;await idbDel('sessions',id);state.sessions=state.sessions.filter(m=>m.id!==id);if(state.activeSessionId===id)state.activeSessionId=null;await audit('session.deleted','Session deleted',{id});render()}
async function sendClientCare(id){const m=state.sessions.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveSession(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.sessions.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveSession(m,'event added');render()}
async function addFileRef(id){const m=state.sessions.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveSession(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.sessions.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveSession(m,'file ref removed');render()}
async function issueAdvice(sessionId){
 const m=state.sessions.find(x=>x.id===sessionId);if(!m)return;
...
async function runConflictCheck(sessionId){
 ...
 await saveSession(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— Fallphysio T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeSession();const cl=m?sessionClient(m):null;
 const ctx=m?`\nActive session: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK physical therapy research assistant for HCPC-regulated physiotherapists. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated physical therapy advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Fallphysio T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('fallphysio-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,sessions:state.sessions,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('fallphysio-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.sessions.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('sessions',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.sessions=state.sessions.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeSessionId&&!state.sessions.find(m=>m.id===state.activeSessionId))state.activeSessionId=null;
 await audit('demo.purged','Demo data removed',{sessions:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Fallphysio booted',{version:VERSION,prime:PRIME});
})();
